...
class GeminiAPI {
  constructor() {
    this.baseUrl = "https://gemini.google.com/_/BardChatUi/data/assistant.lamda.BardFrontendService/StreamGenerate?bl=boq_assistant-bard-web-server_20260630.21_p0&f.sid=8436398543548529905&hl=ar&_reqid=5000358&rt=c";
    this.headers = {
  ...
  async chat({ prompt }) {
    ...
    body.append("at", "AD1_LW7-ykRJHHakn9J5HG54ejJz:1783026355300");

    const response = await axios.post(this.baseUrl, body, { headers: this.headers, timeout: 30000 });
    
    // البحث عن جميع المداخل النصية في الاستجابة (Stream response)
    const matches = [...response.data.matchAll(/\["rc_.*?",\["(.*?)"\]/g)];
    // نأخذ آخر نص عثرنا عليه لأنه الأطول والأكثر اكتمالاً
    return matches.length > 0 ? matches[matches.length - 1][1].replace(/\\n/g, '\n') : "Error: Could not parse response.";
  }
}
