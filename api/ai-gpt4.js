...
class ChatGPT4O {
    ...
    async chat(msg) {
        try {
            ...
            const clientId = this.generateRandomString(10);
...
export default async function handler(req, res) {
    // --- بداية قسم التوافقية (لا يغير في منطق السكراب) ---
    let queryParams = {};
    if (req.query) {
    ...
    if (!prompt) {
        return sendResponse(200, {
            ...
            API: "GPT-4o API Chat",
            Dev: "TanjiroDev ✨🖤",
            usage: { endpoint: "/api/ai/chatgpt", parameters: ["q", "prompt"] }
        });
    }
